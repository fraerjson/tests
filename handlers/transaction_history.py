...
async def display_events(callback: types.CallbackQuery):
    ...
    inline_kb = types.InlineKeyboardMarkup(row_width=1)
    inline_kb.add(types.InlineKeyboardButton("В меня пользователя", callback_data='return_user'))
    await callback.message.answer(response.json())
    await callback.message.answer('В меню пользователя', reply_markup=inline_kb)
...
def setup(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(display_events, Text(contains="get_transaction"))
